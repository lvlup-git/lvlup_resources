...
local function InputWithdraw(amount)
    local response = lib.inputDialog('Withdrawal', {
        {
            ...
            description = ('You have $%s available to withdraw'):format(amount),
            required = true
        },
        {
            type = 'select',
            label = 'Withdraw location',
            required = true,
    ...
    if inputAmt < 1 then
        return DoNotification('Amount needs to be greater than 0.', 'Transaction Error')
    end

    if inputAmt > amount then
        return DoNotification('Insufficient funds in your paycheck.', 'Transaction Error')
    end
